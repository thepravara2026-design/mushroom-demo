...
(async () => {
  try {
    ...
    // Enter OTP
    const step2Visible = await page.$eval('#auth-verify-view', el => !el.classList.contains('hidden'));
    ...
    const modalClosed = await page.$eval('#auth-modal', el => !el.classList.contains('open'));
