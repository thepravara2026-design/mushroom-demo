...
async function run() {
  ...
  let otpRes = await api('/auth/request-otp', {
    method: 'POST',
    body: JSON.stringify({ email: testEmail, role: 'buyer', fullName: 'E2E Test Buyer' })
  }).catch(e => fail(e.message));
  pass(`OTP requested for ${testEmail}: ${JSON.stringify(otpRes)}`);

  // In mock mode, OTP is stored in-memory. Default is '123456'
  let verifyRes = await api('/auth/verify-otp', {
    method: 'POST',
    body: JSON.stringify({ email: testEmail, otpCode: '123456' })
  }).catch(e => fail(`OTP verify failed: ${e.message}`));
  
  if (!verifyRes.token) fail(`No token returned from verify-otp. Got: ${JSON.stringify(verifyRes)}`);
  token = verifyRes.token;
  pass(`Authenticated as ${verifyRes.user.email} (role: ${verifyRes.user.role})`);
  pass(`WhatsApp number defaults to: "${verifyRes.user.whatsappNumber}" (should be empty string)`);
  if (verifyRes.user.whatsappNumber !== '') fail(`whatsappNumber should be '' but got: ${verifyRes.user.whatsappNumber}`);
  ...
  pass(`/me returned user: ${me.email}, role: ${me.role}, whatsappNumber: "${me.whatsappNumber}"`);
  ...
  pass(`spawn: ${spawnProducts.length} | fresh: ${freshProducts.length} | dry: ${dryProducts.length} | kits: ${kitProducts.length}`);
  ...
  pass(`Key: ${checkoutData.razorpay.keyId} (mock: ${orderId.includes('mock') ? 'YES ✓' : 'NO'})`);
  ...
  const promoCheckout = await api('/orders/checkout', {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` },
    body: JSON.stringify({ items: [{ id: products[0].id, quantity: 1 }], promoCode: 'SPORE10' })
  }).catch(e => fail(e.message));
  pass(`Promo checkout: ₹${(promoCheckout.razorpay.amount / 100).toFixed(2)} (should be 10% less than ₹${products[0].price})`);
  ...
  pass(`Admin login OK: ${adminLogin.user.email} (role: ${adminLogin.user.role})`);
  ...
  pass(`Admin successfully fetched all orders. Total count: ${allOrders.length}`);
  ...
  if (!testOrder) fail(`Admin could not find the created order ${completedOrderId} in all orders list.`);
  pass(`Found created order ${completedOrderId} in admin list. Associated buyer email: ${testOrder.user_email}`);
  ...
  if (updateStatusRes.order.delivery_status !== 'shipped') {
    fail(`Delivery status was not updated to shipped. Got: ${updateStatusRes.order.delivery_status}`);
  }
  pass(`Order ${completedOrderId} delivery status successfully updated to "shipped" by Admin.`);
  ...
  const newCat = await api('/categories', {
    method: 'POST',
    headers: { Authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({ id: 'test-cat', name: 'Test Category', description: 'Testing categories CRUD' })
  ...
  const updatedCat = await api(`/categories/${newCat.id}`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({ name: 'Test Category Updated', description: 'Testing categories CRUD updated description' })
  }).catch(e => fail(`Failed to update category: ${e.message}`));
  pass(`Category updated: ${updatedCat.name} (desc: ${updatedCat.description})`);
