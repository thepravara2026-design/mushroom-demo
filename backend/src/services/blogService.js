...
async function createBlog(blogData) {
  const { blog_id, title, content, featured_image, image_source = 'upload', author = 'Admin' } = blogData;

  const slug = generateSlug(title);
  const existing = await db.from('blogs').select('id').eq('slug', slug).single();
  if (existing.data) {
    throw AppError.conflict('A blog with this title already exists');
  }

  const newBlog = {
    id: blog_id || `blog-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    title,
    slug,
    author,
    content,
  ...
  const { data, error } = await db.from('blogs').insert(newBlog).select().single();
...
async function updateBlog(id, updates) {
  ...
  if (updates.title && updates.title !== blog.title) {
    ...
    const existing = await db.from('blogs').select('id').eq('slug', newSlug).neq('id', id).single();
    if (existing.data) {
      throw AppError.conflict('A blog with this title already exists');
    }
...
async function checkAndUpdateLockStatus() {
  ...
  const { data, error } = await db
    .from('blogs')
    .update({ locked: true, status: 'locked', updated_at: new Date().toISOString() })
    .eq('status', 'published')
  ...
  if (error) {
    console.error('Failed to update blog lock status:', error);
  }
...
function startBlogLockScheduler() {
  if (db.isMock) return;
  console.log(`🔒 Blog auto-lock scheduler started (every ${LOCK_CHECK_INTERVAL_MS / 60000} min)`);
